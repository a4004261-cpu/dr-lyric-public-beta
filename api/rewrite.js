...
module.exports = async function handler(req, res) {
  ...
  try {
    const { lyrics = '', mode = '総合診察', diagnosis = null, styleProfile = null } = req.body || {};
...
styleProfileがある場合は、過去の診断傾向とAI編集の採用/拒否傾向を参考にする。特に原文維持率が高ければ変更量を抑え、ユーザーが繰り返し守っている個性を優先する。ただし過去平均へ矯正しない。今回の作品が過去と違う方向へ進んでいる可能性も尊重する。
...
形式: {"edited_lyrics":"...","summary":"...","changes":[{"line":1,"before":"...","after":"...","reason":"...","severity":0-100,"confidence":0-100,"metric_delta":{"flow":0,"rhyme_design":0,"phonetic_pleasure":0,"word_density":0,"punchline":0,"imagery":0,"voice":0,"surprise":0,"repetition_variation":0,"memorability":0,"edge":0,"magnetism":0}}]}`;
    ...
    const profileText = styleProfile ? JSON.stringify(styleProfile).slice(0, 3500) : '履歴なし';
    const prompt = `診察モード: ${mode}\nユーザー作風プロフィール: ${profileText}\n既存診断: ${diagnosisText}\n\n原文:\n${lyrics}\n\n重要: 変更のための変更はしない。原文が強い場合は残す。signature_linesやEDGEの高い行を綺麗に均さない。ユーザーが過去にAI編集を多く拒否している場合は特に変更を慎重にする。変更箇所だけchangesへ入れる。`;
